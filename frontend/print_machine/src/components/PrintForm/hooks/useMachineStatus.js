...
export function useMachineStatus() {
  ...
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const machineFromUrl = params.get("machine");

    let resolvedId = "";

    if (machineFromUrl) {
      // QR code was scanned — save machine ID and immediately clean the URL
      // User will see quickprintdemo.in with no machine ID visible
      sessionStorage.setItem("kiosk_machine_id", machineFromUrl);
      resolvedId = machineFromUrl;
      window.history.replaceState({}, "", window.location.pathname);
    } else {
      // No QR param — check if we already stored it this session (e.g. page reload)
      const stored = sessionStorage.getItem("kiosk_machine_id");
      if (stored) {
        resolvedId = stored;
      }
    }
    ...
    if (resolvedId) {
      ...
      fetchStatus(resolvedId);
      const interval = setInterval(() => fetchStatus(resolvedId), 30000);
      return () => clearInterval(interval);
    } else {
      setError("Invalid kiosk link. Machine not specified.");
    }
